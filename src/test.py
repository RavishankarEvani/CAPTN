...
from src.utils import percentage_acc
...
def test_epoch(
    ...
    with torch.no_grad():
        for batch_idx in prog_bar:
            ...
            output, obfm_out_comb, oelta_out_comb = model(x = data)
            
            current_total_test_loss += criterion(inputs = output, 
                                                 targets = target, 
                                                 orderless_bf_combined = obfm_out_comb, 
                                                 orderless_elta_combined = oelta_out_comb, 
                                                 spatial_latent_attribute = None, 
                                                 extracted_patches = None).item()
